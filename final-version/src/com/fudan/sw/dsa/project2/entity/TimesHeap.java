...
public class TimesHeap {
    private int d = 2;
    private ArrayList<String> names;
    private ArrayList<Address> vertices;
    public int size;

    public TimesHeap(ArrayList<Address> arrayList) {
    ...
    private void minHeapify(ArrayList<Address> vertices, ArrayList<String> names, int i, int length) {
        ...
        for (int j = left; j <= right; j++) {
            if (j <= length - 1 && vertices.get(j).getTimes() < leastValue.getTimes()) {
                least = j;
    ...
    public void decreaseKey(Address vertex) {
        ...
        this.vertices.get(index).setTimes(vertex.getTimes());
        int parent = (int) Math.floor((index - 1.0) / d);
        while (index > 0 && this.vertices.get(parent).getTimes() > this.vertices.get(index).getTimes()) {
