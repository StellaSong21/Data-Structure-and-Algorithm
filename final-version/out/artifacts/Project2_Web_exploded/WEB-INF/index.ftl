...
<div style="width:20%; height:100%;display:inline-block; ">
    <form style="position:absolute;font-size:16px;top:5%;background: hsla(0,0,100%,.16)" id="submitForm">
        ...
        <input style="position:relative;left:40%;width:80px;height:30px;cursor:pointer;border-width: 0;border-radius: 3px;
        outline: none;font-family: Microsoft YaHei;" onmouseover="this.style.color='#0F0'"
    ...
    <div style="position: absolute;top: 40%;width: 20%;height: 60%">
        <span style="font-size: 18px;position: relative;font-family: Georgia;">display the result:</span><br/>
        <div id="resultDiv"
             style="background-color:#F5F5F5;overflow-y:auto;width: 90%;height:90%;font-size:18px;word-wrap: break-word; ">
        </div>
    </div>
</div>
</body>
...
<script type="text/javascript">
    ...
    function clickButton() {
        //请在这里检查数据
        var startAddrass = $("#startAddress").val();
        var endAddress = $("#endAddress").val();
        var choose = $('input[name=items]:checked', '#submitForm').val();
        // alert(startAddrass);
        var warning = "";
        var check = 0;
        if (startAddrass === "") {
        ...
        if (check !== 0) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    let str = "搜索时间为: &nbsp;&nbsp" + data.queryTime + "&nbsp;纳秒<br/>";
                    str += "花费时间约为: &nbsp;&nbsp; " + data.minutes + "&nbsp;分钟<br/>";
                    str += "步行长度约为: &nbsp;&nbsp;" + data.lengths + "&nbsp;米<br/>";

                    str += data.startPoint.address;
                    for (let i = 0; i < len; ++i)
